...
**About the AI Investigation panel:** with `GEMINI_API_KEY` set (a free
key from https://aistudio.google.com/apikey), `make pipeline` produces
real Gemini-written narratives -- badge reads "Generated by Gemini." This
was verified live end to end, all 17 events + all 172 chargeback drafts.
Without a key, every event shows "Deterministic fallback (LLM
unavailable)" instead -- the narrative text is still evidence-complete,
just template-written, and that's a legitimate thing to show too
(section 43's failure handling, working on camera, not asserted). Either
is fine for the video; showing the real Gemini narrative is more
impressive, but the fallback path is the more unusual thing to point at.
Note: with a key, the chargeback responder step of `make pipeline` takes
~12 minutes (172 cases, paced to stay under the free tier's 15
requests/minute) -- run it well before recording, not live on camera.
...
Gemini -- but it only narrates evidence a deterministic engine already
