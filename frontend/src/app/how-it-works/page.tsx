  ...
  return (
    <div className="flex gap-4">
      <div className="flex flex-col items-center shrink-0">
        <div className="w-9 h-9 rounded-full bg-brand-600 text-white flex items-center justify-center text-sm font-display font-semibold">
          {number}
        </div>
        <div className="w-px flex-1 bg-line my-2" />
      </div>
      <div className="pb-8">
...
export default function HowItWorksPage() {
  return (
    <DashboardLayout>
      <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-ink mb-4">
        <ArrowLeft className="w-4 h-4" /> Back to Command Center
      </Link>

      <h1 className="text-2xl font-display font-semibold text-ink tracking-tight mb-2">How LossGraph Works</h1>
      <p className="text-sm text-slate-600 mb-8 max-w-3xl">
        ...
        is entering a <strong className="text-ink font-semibold">Loss Event</strong> -- a coordinated pattern across customers, devices, and
        time -- and works out the cheapest effective way to stop it. Everything on this page reflects the
        actual pipeline: <code className="bg-canvas px-1.5 py-0.5 rounded text-xs font-mono">data/generation/</code> and{' '}
        <code className="bg-canvas px-1.5 py-0.5 rounded text-xs font-mono">ml/</code> in the repo, not a mockup.
      </p>

      <div className="bg-surface rounded-xl shadow-card p-6 mb-10">
        <StepCard number={1} icon={Network} title="Three engines look at the same data from different angles">
          ...
          <ul className="space-y-1.5 ml-1">
            <li><strong className="text-ink">Transaction risk model</strong> (LightGBM) -- catches obvious fraud at the moment of
            ...
            <li><strong className="text-ink">Entity graph engine</strong> (NetworkX) -- builds a network of which customers share
            ...
            <li><strong className="text-ink">Temporal anomaly engine</strong> -- watches each merchant's daily return/dispute rate
        ...
        <StepCard number={2} icon={Brain} title="The three scores are fused, not averaged">
          <p>
            <code className="bg-canvas px-1.5 py-0.5 rounded text-xs font-mono">P_fused = 1 - (1-P₁)(1-P₂)(1-P₃)</code> --
            any single engine being confident is enough to raise the fused score, while it only stays low
            if all three independently see nothing. This is what the <strong className="text-ink">"How the confidence score was
        ...
        <StepCard number={3} icon={GitBranch} title="A Loss Event is formed -- not a flagged transaction">
          <p>
            ...
            merchant-day whose outcome rate spiked get bundled into one <strong className="text-ink">Loss Event</strong>, with a
        ...
        <StepCard number={6} icon={Receipt} title="A later chargeback links straight back">
          <p>
            ...
            high-confidence Loss Event, the recommendation is to <strong className="text-ink">accept</strong> the chargeback, not
        ...
        <div className="flex gap-4">
          ...
          <div>
            <h3 className="text-base font-display font-semibold text-ink mb-1">That's the whole loop</h3>
      ...
      <div className="bg-surface rounded-xl shadow-card p-6 mb-10 border-l-4 border-brand-600">
        <h2 className="text-lg font-display font-semibold text-ink mb-4">The honest numbers (held-out test split)</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div>
            <p className="text-2xl font-display font-semibold text-brand-700 tabular-nums">0.61</p>
            <p className="text-xs text-slate-500 mt-1">Fused PR-AUC -- beats every single engine alone</p>
          </div>
          <div>
            <p className="text-2xl font-display font-semibold text-brand-700 tabular-nums">79%</p>
            <p className="text-xs text-slate-500 mt-1">Gross loss prevented at the economically-tuned threshold</p>
          </div>
          <div>
            <p className="text-2xl font-display font-semibold text-brand-700 tabular-nums">4.71 vs 0.70</p>
          ...
          <div>
            <p className="text-2xl font-display font-semibold text-brand-700 tabular-nums">100%</p>
            <p className="text-xs text-slate-500 mt-1">
        ...
        <p className="text-xs text-slate-400 mt-5 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5" /> From the current <code className="bg-canvas px-1.5 py-0.5 rounded font-mono">make pipeline</code> run.
          Full methodology and honestly-stated limitations in <code className="bg-canvas px-1.5 py-0.5 rounded font-mono">docs/EVALUATION.md</code>.
        </p>
      </div>
